...
def get_prediction(image):
    """Get the prediction from the model for an image"""
    output = model(image)
    _, prediction = torch.max(output, 1)
    return prediction.item()

def get_predicted_label(prediction):
    """Get the predicted label based on the prediction"""
    if prediction >= len(labels):
        return "The prediction made by the model is incorrect."

    predicted_label = labels[prediction]
    return predicted_label

def get_suggestions(prediction):
    """Get five suggestions based on the prediction"""
    suggestions = random.sample(labels, 5)
    suggestions.insert(0, labels[prediction])
    return suggestions
...
while True:
    ...
    if prediction.item() >= len(labels):
        print("The prediction made by the model is incorrect.")
        continue

    predicted_label = labels[prediction.item()]
    print(f"The model predicted: {predicted_label}")
    suggestions = random.sample(labels, 5)
    suggestions.insert(0, predicted_label)
    print("Suggestions:")
    for i, suggestion in enumerate(suggestions):
      print(f"{i + 1}. {suggestion}")

    label = input("Choose a label (input name or number 1-6):")
    if label.isdigit():
        label = int(label) - 1
        if label >= 0 and label <= 5:
            labels[prediction.item()] = suggestions[label]
    else:
        if label in labels:
            labels[prediction.item()] = label
        else:
            labels.append(label)
    if input("Train on another image? (yes/no)").lower() == "no":
        break
